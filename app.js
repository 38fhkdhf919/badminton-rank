...
// =========================================================================
// 🧪 [마스터 치트키] 5대 매칭 로직 검증용 무한 궤도 시뮬레이터
// =========================================================================
window.runFullTournamentSimulation = function() {
    console.clear();
    console.log("🚀 [시뮬레이터] 5대 코어 매칭 로직 30경기 마스터 테스트 가동...");

    // 1. 가상 참가자 24명 라인업 세팅 (MMR 900 ~ 2200 대역 골고루 분포)
    const virtualPlayers = Array.from({ length: 24 }, (_, i) => ({
        id: 100 + i,
        name: `가상회원_${String.fromCharCode(65 + (i % 26))}${i + 1}`,
        mmr: 1000 + (i * 50) + Math.floor(Math.random() * 30), // 1000 ~ 2200점대 분포
        actualMatchCount: 0,
        restPenalty: 0
    }));
    ...
    // 가상의 세션 스냅샷 객체 생성
    let s = {
        attendees: virtualPlayers.map(p => p.id),
        restPlayers: [],
        currentMatches: [],
        historyLog: [],
        courtHoldTimestamp: null,
        playerMatchCounts: {},
        playerLastOutTimestamps: {}
    };

    // 중간에 피로도가 쌓여 쉼터로 이동했다가 복귀하는 극단적 유저 설정 (관리자님이 우려하신 케이스)
    const lazyPlayerId = 105; // 초반 2경기 후 쉼터로 도망갈 유저

    // 2. 30경기 무한 자동 대진 가상 루프 가동 (4개 코트 동시 가동 상황 모사)
    for (let gameIdx = 1; gameIdx <= 30; gameIdx++) {
        
        // [시나리오 주입]: 5번째 경기 때 특정 유저가 힘들어서 쉼터로 탈탈 이동
        if (gameIdx === 5) {
            s.restPlayers.push(lazyPlayerId);
            console.log(`\n💤 [시나리오] ${virtualPlayers.find(p => p.id === lazyPlayerId).name}님이 지쳐서 쉼터로 대기열 제외 처리됨.`);
        }
        
        // [시나리오 주입]: 남들 경기 더 뛰고 난 15번째 경기 때 다시 복귀 합류!
        if (gameIdx === 15) {
            s.restPlayers = s.restPlayers.filter(id => id !== lazyPlayerId);
            console.log(`\n🔥 [시나리오] 쉼터에서 쉬던 ${virtualPlayers.find(p => p.id === lazyPlayerId).name}님이 대기열로 기습 복귀!!`);
        }

        // 현재 뛰고 있는 유저 바인딩
        const busyPlayerIds = new Set();
        s.currentMatches.forEach(m => {
            m.teamA.forEach(id => busyPlayerIds.add(id));
            m.teamB.forEach(id => busyPlayerIds.add(id));
        });

        // 실시간 실력 변동에 따른 랭킹 테이블 라인업 재정렬
        const rankedPlayers = [...virtualPlayers]
            .filter(p => s.attendees.includes(p.id))
            .sort((a, b) => b.mmr - a.mmr)
            .map((p, index) => ({ ...p, currentRank: index + 1 }));

        // 대기조 풀 컴파일
        const activeQueueIds = s.attendees.filter(id => !s.restPlayers.includes(id));
        const queuePool = activeQueueIds
            .map(id => {
                const systemInfo = rankedPlayers.find(x => x.id === id);
                const actualCount = s.playerMatchCounts[id] || 0;
                
                // 💡 대안 1 로직 실시간 가동
                let penalty = 0;
                if (s.restPlayers.includes(id)) {
                    const maxGap = s.historyLog.length - actualCount;
                    penalty = maxGap > 0 ? maxGap : 0;
                }

                return {
                    id,
                    name: systemInfo.name,
                    rank: systemInfo.currentRank,
                    mmr: systemInfo.mmr,
                    effectiveGameCount: actualCount + penalty,
                    lastOutTime: s.playerLastOutTimestamps[id] || 0,
                    isBusy: busyPlayerIds.has(id)
                };
            })
            .filter(p => !p.isBusy);

        if (queuePool.length < 4) continue;

        // 보정 경기수 최저자 정렬
        queuePool.sort((a, b) => {
            if (a.effectiveGameCount !== b.effectiveGameCount) return a.effectiveGameCount - b.effectiveGameCount;
            return a.lastOutTime - b.lastOutTime;
        });

        // 슬라이딩 윈도우 규칙 적용 (±4등 가드)
        const anchorPlayer = queuePool[0];
        let allowedDiff = (anchorPlayer.rank <= 4 || anchorPlayer.rank >= rankedPlayers.length - 3) ? 8 : 4;

        const fitCandidates = queuePool.filter(p => Math.abs(p.rank - anchorPlayer.rank) <= allowedDiff);
        let selectedFour = null;

        if (fitCandidates.length >= 4) {
            let minRankRange = 999;
            let bestSubPool = null;

            for (let i = 0; i < fitCandidates.length; i++) {
                for (let j = i + 1; j < fitCandidates.length; j++) {
                    for (let k = j + 1; k < fitCandidates.length; k++) {
                        const candidateFour = [anchorPlayer, fitCandidates[i], fitCandidates[j], fitCandidates[k]];
                        const uniqueSet = new Set(candidateFour.map(x => x.id));
                        if (uniqueSet.size !== 4) continue;

                        const ranks = candidateFour.map(x => x.rank);
                        const range = Math.max(...ranks) - Math.min(...ranks);

                        if (range < minRankRange) {
                            minRankRange = range;
                            bestSubPool = candidateFour;
                        }
                    }
                }
            }
            selectedFour = bestSubPool;
        }

        // 윈도우 격차 미달 시 매칭 홀딩 처리 진행
        if (!selectedFour || selectedFour.length < 4) {
            console.log(`⚠️ [제 ${gameIdx}경기]: 랭킹 균형 불일치로 코트 매칭 홀딩 작동됨.`);
            continue; 
        }

        // 2:2 최소 격차 팀 분배
        const [p1, p2, p3, p4] = selectedFour;
        const case1 = { tA: [p1, p2], tB: [p3, p4], diff: Math.abs((p1.mmr + p2.mmr) - (p3.mmr + p4.mmr)) };
        const case2 = { tA: [p1, p3], tB: [p2, p4], diff: Math.abs((p1.mmr + p3.mmr) - (p2.mmr + p4.mmr)) };
        const case3 = { tA: [p1, p4], tB: [p2, p3], diff: Math.abs((p1.mmr + p4.mmr) - (p2.mmr + p3.mmr)) };
        const bestCase = [case1, case2, case3].sort((a, b) => a.diff - b.diff)[0];

        // 경기 가상 마감 및 스코어 처리 (난수)
        const finalScoreA = Math.random() > 0.5 ? 25 : Math.floor(Math.random() * 5) + 20;
        const finalScoreB = finalScoreA === 25 ? Math.floor(Math.random() * 10) + 15 : 25;

        const matchRecord = {
            matchIdx: s.historyLog.length + 1,
            teamANames: bestCase.tA.map(x => `${x.name}(${x.rank}등)`).join(', '),
            teamBNames: bestCase.tB.map(x => `${x.name}(${x.rank}등)`).join(', '),
            score: `${finalScoreA} : ${finalScoreB}`,
            rankRange: Math.max(...selectedFour.map(x => x.rank)) - Math.min(...selectedFour.map(x => x.rank))
        };

        // 전적 업데이트
        selectedFour.forEach(p => {
            s.playerMatchCounts[p.id] = (s.playerMatchCounts[p.id] || 0) + 1;
            s.playerLastOutTimestamps[p.id] = gameIdx;
            
            // 승패에 따른 가상 ELO 정산 (단순 계산 모사)
            const isWinA = finalScoreA > finalScoreB;
            const isTeamA = bestCase.tA.some(x => x.id === p.id);
            const pObj = virtualPlayers.find(v => v.id === p.id);
            if (pObj) {
                pObj.actualMatchCount++;
                if ((isWinA && isTeamA) || (!isWinA && !isTeamA)) {
                    pObj.mmr += 12; // 승리
                } else {
                    pObj.mmr -= 10; // 패배
                }
        ...
        s.historyLog.push(matchRecord);
    }

    // 3. ✨ [결과 리포트 도출] 콘솔에 한눈에 보이게 대진 일지 인쇄
    console.log("\n📊 ==================== [시뮬레이션 완료 대진 일지] ====================");
    console.table(s.historyLog.map(h => ({
        "경기 번호": `제 ${h.matchIdx}경기`,
        "TEAM A (등수)": h.teamANames,
        "스코어 결과": h.score,
        "TEAM B (등수)": h.teamBNames,
        "4인 최고-최저 등수 격차": `${h.rankRange}등 차이 (품질)`
    })));

    console.log("\n👥 가상 유저별 최종 전적 및 연참 조율 결과 (중간에 쉰 유저 확인용):");
    console.table(virtualPlayers.sort((a,b)=>b.mmr - a.mmr).map(p => ({
        "이름": p.name,
        "최종 MMR": `${p.mmr}점`,
        "실제 소화한 경기 수": `${p.actualMatchCount}회`
    })));
    
    alert("🏆 30경기 풀 시뮬레이션 컴파일 완료! 개발자 도구(F12) 콘솔 창에서 타임라인 일지를 한눈에 대조해 보세요.");
};
